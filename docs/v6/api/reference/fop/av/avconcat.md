...
- 拼接视频片段时要保证所有源画面长宽值一致
- 本接口只能用于[预转持久化处理][persistentOpsHref]和[触发持久化处理][pfopHref]

<a id="avconcat-samples"></a>
## 示例

- 以触发持久化处理形式，将多段mp4视频拼接成一段（分别是thinking-in-go.1.mp4、thinking-in-go.2.mp4、thinking-in-go.3.mp4。 假设这几个视频放在一个绑定了域名`test.clouddn.com`的空间中）：  
...
**注意：**要在Authorization头部的`<AccessToken>`前添加`QBox`和半角空格。
