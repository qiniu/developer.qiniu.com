...
```
<form method="post" action="http://up-glb.qiniu.com/glb"
...
头部名称       | 必填 | 说明
:------------- | :--- | :------------------------------------------
Host           | 是   | 上传服务器域名，固定为`up-glb.qiniu.com`。
...
`<key>`                       |      | 资源的名称，位于`key`消息中，**这个不是最终的名称，上传完毕后会根据最终区域，修改`<key>`，带上`<region>/`的前缀**。如不指定则使用[上传策略][putPolicyHref]saveKey字段所指定模板生成Key，如无模板则使用`<region>/<hash>`值作为Key。
...
`key`    | 是   | 目标资源的最终名字，可由七牛云存储自动命名。**上传完毕后会根据最终区域，修改`<key>`，带上`<region>/`的前缀**
