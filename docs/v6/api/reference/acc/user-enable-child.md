---
layout: docs
title: 启用子账号（user/enable_child）
order: 650
---

<a id="user-enable-child"></a>
# 启用子账号（user/enable_child）

<a id="user-enable-child-description"></a>
## 描述

用户登录父账号后，通过本接口启用指定子账户。  

<font style="color:red;">仅限于父账号使用。</font>

<a id="user-enable-child-request-syntax"></a>
### 请求语法

```
POST /user/enable_child HTTP/1.1
Host:           acc.qbox.me
Content-Type:   application/x-www-form-urlencoded
Authorization:  Bearer <AccessToken>

<UserEnableChildRequestParams>
```

<a id="user-enable-child-request-auth"></a>
### 访问权限

[账号管理凭证](access-token.html#oauth-token-description)。

<a id="user-enable-child-request-headers"></a>
### 头部信息
...
<a id="user-enable-child-request-params"></a>
### 请求参数
...
:------------ | :--- | :--------------------------- | :-----------------------------
uid           | 是   |                              | 启用对象子账号的数字UUID。

<a id="user-enable-child-response"></a>
## 响应

<a id="user-enable-child-response-syntax"></a>
### 响应语法
...
<UserEnableChildResponseContent>
```

<a id="user-enable-child-response-headers"></a>
### 头部信息
...
<a id="user-enable-child-response-body"></a>
### 响应内容
...
<a id="user-enable-child-errors"></a>
...
<a id="user-enable-child-response-status"></a>
### 响应状态码

HTTP状态码 | 含义
:--------- | :--------------------------
200        | 授权成功。
4xx	       | 参考[错误消息表](#user-enable-child-errors)。
599	       | 服务端操作失败。<br>如遇此错误，请将完整错误信息（包括所有HTTP响应头部）[通过邮件发送][sendBugReportHref]给我们。

<a id="user-enable-child-remarks"></a>
## 附注
