...
<a id="put-policy-persistent-ops"></a>`persistentOps`       |      | 资源上传成功后触发执行的预转持久化处理指令列表,<br>添加`persistentPipeline`字段，使用专用队列处理。<br>每个指令是一个API规格字符串，多个指令用“;”分隔。<br>请参看[详解](#put-policy-persistent-ops-explanation)与[示例](#put-policy-samples-persisntent-ops)。
