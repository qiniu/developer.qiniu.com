...
<a id="put-policy-fop-timeout"></a>`fopTimeout`           |      | ● 文件变换操作执行的超时时间（单位：秒）<br>这个值太小可能会导致误判（最终存储成功了但客户端得到超时错），太大可能会导致服务端将其判断为低优先级任务。<br>建议取一个相对准确的时间估计值*N（N不要超过5）。<br>须与`transform`字段配合使用。
