...
<a id="put-policy-persistent-ops"></a>`persistentOps`       |      | 资源成功上传后执行的持久化指令列表,每个指令是一个API规格字符串，多个指令用“;”分隔。<br>请参看[详解](#put-policy-persistent-ops-explanation)与[示例](#put-policy-samples-persisntent-ops)。<br>同时添加`persistentPipeline`字段，使用专用队列处理,请参考[专有队列](#put-policy-persistentPipeline)字段说明。
